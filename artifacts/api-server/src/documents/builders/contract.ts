...
import { rentalDayCount } from "../../lib/bookingIntervals";
...
// Contract-only print palette. It is intentionally darker than the proposal
// palette so small legal text remains legible on screens and office printers.
const GOLD_INK = "#6B4F00";
const LIGHT_GREY = "#E9EDF2";
const DARK_GREY = "#111318";
const MUTED_GREY = "#3F4650";
const HAIRLINE = "#B8BEC7";
...
export function renderContractHtml(input: ContractInput): string {
  ...
  const days =
    input.pickupDate && input.returnDate
      ? rentalDayCount(input.pickupDate, input.returnDate)
      : null;
  const agreedMileage =
  ...
  const css = `
    ...
    * { box-sizing: border-box; }
    ...
    .ctr-company-addr { font-size: 8.8px; font-weight: 600; color: ${MUTED_GREY}; margin-top: 1.5mm; }
    ...
    .ctr-kv-l { background: ${LIGHT_GREY}; font-family: ${HEAD}; font-size: 7.8px; letter-spacing: 0.6px; text-transform: uppercase; color: ${DARK_GREY}; font-weight: 800; width: 34%; }
    .ctr-kv-v { background: ${WHITE}; color: ${DARK_GREY}; font-weight: 600; }
    ...
    .ctr-sig-role { font-size: 8.3px; font-weight: 600; color: ${MUTED_GREY}; margin-top: 1mm; }
    .ctr-sig-date { font-size: 8.3px; font-weight: 600; color: ${MUTED_GREY}; margin-top: 5mm; }
    ...
    .ctr-footer { position: absolute; left: 0; right: 0; bottom: 0; display: flex; justify-content: space-between; align-items: center; font-family: ${HEAD}; font-size: 7.8px; font-weight: 700; letter-spacing: 0.6px; text-transform: uppercase; color: ${DARK_GREY}; border-top: 1px solid ${HAIRLINE}; padding-top: 2.5mm; }
    ...
    .ctr-page-2 .ctr-liability-body { font-size: 9.1px; line-height: 1.5; }
  ...
  const rentalSection =
    ...
    kv(
      "Pick-up",
      `${esc(formatDate(input.pickupDate))}${input.pickupTime ? ` at ${esc(input.pickupTime)}` : ""} — ${esc(input.pickupLocation)}`,
    ) +
    kv(
      "Return",
      `${esc(formatDate(input.returnDate))}${input.returnTime ? ` at ${esc(input.returnTime)}` : ""} — ${esc(input.returnLocation)}`,
    ) +

...
const fadeInRight: Variants = {
  hidden: { opacity: 0, x: 40 },
  visible: { opacity: 1, x: 0, transition: { duration: 0.9, delay: 0.2, ease: [0.25, 0.46, 0.45, 0.94] } },
};

const strip = (s: string) => s.replace(/<[^>]+>/g, "").replace(/&quot;/g, '"').replace(/&amp;/g, "&").replace(/&lt;/g, "<").replace(/&gt;/g, ">").replace(/&nbsp;/g, " ").replace(/&#?\w+;/g, "").trim();
...
export default function About() {
  ...
  useEffect(() => {
    fetchContent(lang).then(setContent).catch(() => {});
  }, [lang]);
    ...
    <div className="min-h-screen bg-background overflow-x-hidden">
      ...
      <section className="min-h-[calc(100vh-64px)] flex items-center relative overflow-hidden">
        <div className="absolute inset-0 overflow-hidden">
          <div className="hidden md:block absolute top-1/4 left-0 w-[500px] h-[500px] bg-gold/[0.04] rounded-full blur-[200px]" style={{ animation: 'glow-pulse 10s ease-in-out infinite' }} />
          <div className="absolute bottom-1/4 right-1/4 w-[400px] h-[400px] bg-gold-dark/[0.03] rounded-full blur-[200px]" style={{ animation: 'glow-pulse 14s ease-in-out infinite 3s' }} />
        </div>
        ...
        <div className="w-full max-w-[1400px] mx-auto px-6 md:px-12 lg:px-16 py-12 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
            ...
            >
              <div className="relative rounded-2xl overflow-hidden aspect-[4/3] lg:aspect-[3/4]">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center p-8 md:p-12">
                  ...
                  <CmsContent as="h1" className="font-antro text-[2.4rem] md:text-[3rem] lg:text-[3.5rem] xl:text-[4rem] text-gold leading-[1.15] text-center whitespace-pre-line" html={slogan} />
                </div>
              </div>
            </motion.div>
            ...
            >
              <div className="mb-6">
                <p className="text-[9px] uppercase tracking-[0.5em] text-gold/50 mb-3 font-light">—</p>
                <CmsContent as="h2" className="font-serif text-2xl md:text-3xl text-white mb-2 tracking-tight" html={aboutTitle} />
                <div className="w-[40px] h-[1px] bg-gold/30 mt-3" />
              </div>
              <div className="mb-10">
                <CmsContent as="div" className="text-white/55 font-light font-wix leading-[1.65] text-[15px] md:text-[16px] space-y-5" html={text} />
              </div>

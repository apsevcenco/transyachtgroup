...
export default function AdminGuides() {
  ...
  const translateDraft = async () => {
    if (plainFleetText(form.title).length < 3 || plainFleetText(form.content).length < 20) {
      setMessage("Generate or write the English article before translating.");
      return;
    }
    setBusy(true); setMessage("");
    try {
      const translations = await translateGuideDraftWithAi({
        title: form.title,
        excerpt: form.excerpt,
        content: form.content,
        metaTitle: form.metaTitle,
        metaDescription: form.metaDescription,
        internalLinks: ai.internalLinks,
        excludeId: editing || undefined,
      });
      setForm((current) => ({ ...current, translations: { ...(current.translations || {}), ...translations } }));
      setMessage("AI translations are ready. Review them, then save the draft.");
    } catch (err) { setMessage(err instanceof Error ? err.message : "AI translation failed"); }
    finally { setBusy(false); }
  };
  ...
  return <div className="min-h-screen bg-[hsl(0,0%,3%)] px-4 py-8 text-white sm:px-8"><div className="mx-auto max-w-6xl">
    ...
    <section className="mb-7 rounded-xl border border-gold/20 bg-gold/[0.04] p-5 md:p-7">
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        <button disabled={busy || ai.topic.trim().length < 5} onClick={generate} className="inline-flex items-center gap-2 rounded bg-gold px-6 py-3 text-sm font-medium text-black disabled:opacity-40"><Sparkles size={16}/>{busy ? "Generating article…" : "Generate AI article draft"}</button>
        <button type="button" disabled={busy || form.title.trim().length < 3 || plainFleetText(form.content).length < 20} onClick={translateDraft} className="inline-flex items-center gap-2 rounded border border-gold/30 px-6 py-3 text-sm font-medium text-gold disabled:opacity-40"><Sparkles size={16}/>{busy ? "Working…" : "Translate article with AI"}</button>
      </div>
    </section>
